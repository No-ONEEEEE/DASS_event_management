...
// Public clubs list for signup/onboarding
router.get('/public/clubs', async (req, res) => {
  try {
    const clubs = await Organizer.find({ isApproved: true })
      .select('_id organizerName category description')
...
router.post('/participant/signup', async (req, res) => {
  try {
    ...
    const selectedClubIds = Array.isArray(followedClubs) ? followedClubs : [];

    let validFollowedClubIds = [];
    if (selectedClubIds.length > 0) {
      const approvedClubs = await Organizer.find({
        _id: { $in: selectedClubIds },
        isApproved: true
      }).select('_id');

      validFollowedClubIds = approvedClubs.map(club => club._id);
    }
    ...
    const participant = new Participant({
      ...
      followedClubs: validFollowedClubIds,
      hasCompletedOnboarding: selectedInterests.length > 0 || validFollowedClubIds.length > 0
    });

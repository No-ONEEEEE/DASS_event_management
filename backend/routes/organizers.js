...
function mapCustomFormToRegistrationFields(customForm) {
...
router.post('/events/create', verifyOrganizer, async (req, res) => {
  try {
    ...
    const normalizedRegistrationFields =
      Array.isArray(registrationFormFields) && registrationFormFields.length > 0
        ? registrationFormFields
        : mapCustomFormToRegistrationFields(customForm);

    const normalizedCustomForm =
      customForm && Array.isArray(customForm.fields)
        ? customForm
        : { fields: [] };
    ...
    const event = new Event({
      ...
      eventTags,
      venue,
      registrationFormFields: normalizedRegistrationFields,
      customForm: normalizedCustomForm,
      isTeamEvent,
      minTeamSize,
      maxTeamSize,
      merchandise,
...
router.put('/events/:eventId', verifyOrganizer, async (req, res) => {
  try {
    ...
    // Keep legacy and new form fields in sync for participant registration UI.
    if (updates.customForm && !updates.registrationFormFields) {
      updates.registrationFormFields = mapCustomFormToRegistrationFields(updates.customForm);
    }

    if (updates.registrationFormFields && !updates.customForm) {
      updates.customForm = {
        fields: (updates.registrationFormFields || []).map(field => ({
...
router.get('/events/:eventId', verifyOrganizer, async (req, res) => {
  try {
    ...
    // Build merchandise lookup for readable item names in participant details.
    const merchNameById = {};
    if (event.merchandise && Array.isArray(event.merchandise.items)) {
      event.merchandise.items.forEach(item => {
        merchNameById[item._id.toString()] = item.itemName;
      });
    }
    ...
    const participants = registrations.map(reg => ({
      ...
      merchandisePurchase: reg.merchandisePurchase
        ? {
            totalAmount: reg.merchandisePurchase.totalAmount || 0,
            paymentStatus: reg.merchandisePurchase.paymentStatus || 'Pending',
            items: (reg.merchandisePurchase.items || []).map(item => ({
              itemId: item.itemId,
              itemName: merchNameById[item.itemId] || 'Item',
...
router.put('/events/:eventId/status', verifyOrganizer, async (req, res) => {
  try {
    ...
    const allowedTransitions = {
      ...
      Published: ['Published', 'Ongoing', 'Closed'],
      Ongoing: ['Ongoing', 'Completed', 'Closed'],

...
const chatUploadsDir = path.join(__dirname, '../frontend/public/uploads/chat-files');
...
const upload = multer({
  ...
  fileFilter: function (req, file, cb) {
    ...
    const mimeAllowed = allowedExactMimes.has(file.mimetype) ||
...
router.post('/team/:teamId/upload', verifyParticipant, upload.single('file'), async (req, res) => {
  try {
    ...
    const sender = await Participant.findById(req.user.id).select('firstName lastName');
    if (!sender) {

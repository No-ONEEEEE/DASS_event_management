...
router.post('/follow/:organizerId', verifyParticipant, async (req, res) => {
  try {
    ...
    if (isFollowing) {
      // Unfollow via atomic updates to avoid unrelated organizer schema validation failures.
      ...
      // Follow via atomic updates to avoid unrelated organizer schema validation failures.
      await Participant.updateOne(
        { _id: req.user.id },

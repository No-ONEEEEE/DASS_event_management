...
router.post('/create', verifyParticipant, async (req, res) => {
  try {
    ...
    const leaderExistingRegistration = await Registration.findOne({
...
router.post('/join/:inviteCode', verifyParticipant, async (req, res) => {
  try {
    ...
    if (team.eventId.status !== 'Published' && team.eventId.status !== 'Ongoing') {
      return res.status(400).json({ message: 'Team registration is not open for this event' });
    }

    if (new Date() > new Date(team.eventId.registrationDeadline)) {
      return res.status(400).json({ message: 'Registration deadline has passed' });
    }

    const memberExistingRegistration = await Registration.findOne({
      participantId: req.user.id,
    ...
    res.json({
      message: teamBecameComplete
        ? 'Team is fully formed. Registrations and tickets have been generated for all members.'
  ...
  try {
    ...
    if (team.status === 'registered') {
      const acceptedMemberIds = [
        team.teamLeaderId._id,
        ...team.members
          .filter(m => m.status === 'accepted' && m.participantId && m.participantId._id)
          .map(m => m.participantId._id)
      ];

      const registrationQuery = (Array.isArray(team.registrationIds) && team.registrationIds.length > 0)
        ? { _id: { $in: team.registrationIds } }
        : (team.registrationId
          ? { _id: team.registrationId }
          : {
              eventId: team.eventId._id,
              teamName: team.teamName,
              participantId: { $in: acceptedMemberIds }
            });

      const teamRegistrations = await Registration.find(registrationQuery)
        .select('_id participantId ticketId')
        .populate('participantId', 'firstName lastName email')
        .lean();

      teamData.memberTickets = teamRegistrations.map(reg => ({
        participantId: reg.participantId?._id,
        participantName: reg.participantId
          ? `${reg.participantId.firstName} ${reg.participantId.lastName}`
          : 'Member',
        participantEmail: reg.participantId?.email || '',
        registrationId: reg._id,
        ticketId: reg.ticketId,
        ticketLink: `/ticket/${reg._id}`,
        isMine: reg.participantId?._id?.toString() === req.user.id
      }));
    } else {
      teamData.memberTickets = [];
    }
...
async function createTeamRegistration(team) {
  try {
    ...
    // Ensure no member already has a registration for this event.
    const existingRegistrations = await Registration.find({
      eventId: team.eventId,
      participantId: { $in: allMemberIds },
      status: { $ne: 'Cancelled' }
    ...
    // Create one ticket/registration per accepted team participant.
    ...
    // Keep backward compatibility with screens expecting a single registrationId.
    team.registrationId = createdRegistrations[0]?._id;
    team.registrationIds = createdRegistrations.map(r => r._id);
    team.status = 'registered';
    await team.save();

    // Update event registration count by fully formed member count.
    event.currentRegistrations += allMemberIds.length;
    await event.save();
